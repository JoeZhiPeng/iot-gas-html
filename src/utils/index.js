 ...
 * @LastEditTime: 2019-04-24 16:08:38
 */
...
export const utils = {
     ...
     * @return <Array> 地区数据
     */
    getAreaTree: function(callback) {
        instance.get('/proxy/areaTree', {}).then(res => {
            if (res.status == 200) {
    ...
    getTableHeight: function(callback) {
        ...
        let t1 = window.setTimeout(() => {
            let h = $('.apps').height();
            let h3 = $('.navbar').height();
            let h1 = $('.filter-container').height(); //搜索栏
            let h2 = 32; // 页码高度
            callback(h - (h1 || 0) - h2 - (h3 || 0) - 65); //92主要是所以margin之和
