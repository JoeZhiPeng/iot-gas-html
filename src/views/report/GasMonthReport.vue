 ...
 * @LastEditTime: 2019-04-26 15:31:07
 -->
<template>
    <div>
        <!-- 搜索条件 -->
        ...
        <!-- 表格 -->
        <el-table ref="multipleTable" :data="list" class="monthTable" :height="height" border fit highlight-current-row element-loading-text="拼命加载中">
            ...
            <el-table-column align="center" label="用气量">
                <template slot-scope="scope">
					<span v-if="scope.row.gas">{{scope.row.gas}}m³</span>
					<span v-else>--</span>
				</template>
            </el-table-column>
			...
			getLine() {
				...
				initEcharts().then(echarts => {
					...
					vm.mileMonthLine.setOption({
						...
						series: [{
							data: [250,180,159,200,124,215,225, 224,200, 250, 235, 225, 240],
							type: 'bar',
							label: {
								normal: {
									show: true,
