 ...
 * @LastEditors: qiaozp
 * @Description: 组织机构下拉框
 * @Date: 2019-04-09 19:56:45
 * @LastEditTime: 2019-04-15 09:28:55
 -->
 
<template>
	<!-- 组织机构下拉框 -->
	<el-select v-model="deptNames" placeholder="请选择" @clear="deptClear" clearable>
    	<el-option disabled label="请选择" value="0" style="display:none"></el-option> 
    	<el-input placeholder="请选择" class="select-tree-input" v-model="filterText" clearable></el-input> 
	    <el-tree
			ref="deptTree"
			:data="deptList"
...
<script>
	...
	export default {
		...
		methods: {
	        ...
	        initDepart() {
	        	const vm = this;
	        	if(vm.$store.getters.departData.length == 0 ){
	        		vm.$store.dispatch('GetDepartData').then(res => { 
			            vm.deptList = vm.$store.getters.departData;
			        }).catch(err => {
			            console.log(err);
			        });
	        	}else{
			...
			deptClear() {
				...
				//修改第一次进入，直接点清除后，setCheckedKeys没有触发change
				this.deptNames = "";
				this.deptId = "";
				this.$emit('deptIdChange', "");
			},
